...
sim_result <- foreach(k = 1:n_sim, .combine = cbind, .packages = c("tidyverse", "lme4", "MASS")) %dopar% {
  ...
  A <- rep(0, m)
  if(sum(S)/2 == round(sum(S)/2)){
    A[S == 1] <- sample(c(rep(1, sum(S==1) * 0.5), rep(0, sum(S==1) * 0.5)), size = sum(S == 1), replace = F)
    A[S == 0] <- sample(c(rep(1, sum(S==0) * 0.5), rep(0, sum(S==0) * 0.5)), size = sum(S == 0), replace = F)
  } else {
    A[S == 1] <- sample(c(rep(1, sum(S==1) * 0.5), rep(0, sum(S==1) * 0.5), rbernoulli(1)), size = sum(S == 1), replace = F)
    A[S == 0] <- sample(c(rep(1, sum(S==0) * 0.5), rep(0, sum(S==0) * 0.5), rbernoulli(1)), size = sum(S == 0), replace = F)
    
  }
  sim_data <- map_dfr(1:m, function(i){
    x <- rnorm(n, sd = 2)
    y <- 2*S[i]*(A[i] + x + mean(x)) + rnorm(n, sd = 5)
  ...
  mix_fit <- lmer(outcome~treatment+x+S + (1|cluster), data = sim_data, REML = F)
  mix_fit_reml <- lmer(outcome~treatment+x+S + (1|cluster), data = sim_data, REML = T)
  ...
  cl_fit <- lm(outcome~treatment+x+S, data = cluster_level_data)
...
print("Scenario 2")
summary_result <- data.frame(bias = apply(sim_result[c(1,3,5,7,9,11),], 1, mean)-1.2,
                             se = apply(sim_result[c(1,3,5,7,9,11),], 1, sd),
                             ase = apply(sim_result[c(2,4,6,8,10,12),], 1, mean),
                             cp = map_dbl(c(1,3,5,7,9,11), function(j){
                               mean(abs(sim_result[j,]-1.2) - qnorm(0.975, mean=0, sd=sim_result[j+1,]) <=0)
                             }))
